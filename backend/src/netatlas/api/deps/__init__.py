...
def device_to_dict(device: Any) -> dict[str, Any]:
    return {
        ...
        "first_seen_at": device.first_seen_at.isoformat() if getattr(device, "first_seen_at", None) else None,
        "network_role": getattr(device, "network_role", None) or "unknown",
        "role_confidence": float(getattr(device, "role_confidence", 0) or 0),
        "role_reasons": list(getattr(device, "role_reasons", None) or []),
        "role_source": getattr(device, "role_source", None) or "auto",
        "location": getattr(device, "location", None),
        "rack": getattr(device, "rack", None),
        "owner": getattr(device, "owner", None),
        "criticality": getattr(device, "criticality", None) or "normal",
